...
def get_groq_api_key():
    # 1. Try Streamlit Cloud secrets (Works only online)
    try:
        if "GROQ_API_KEY" in st.secrets:
            return st.secrets["GROQ_API_KEY"]
    except Exception:
        pass  # st.secrets doesn't exist locally
    ...
    # 3. If still missing → Raise clear error
    raise RuntimeError("GROQ_API_KEY not found! "
                       "Put it in .env locally or Streamlit Cloud Secrets.")
